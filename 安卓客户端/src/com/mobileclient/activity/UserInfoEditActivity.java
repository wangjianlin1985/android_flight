...
public class UserInfoEditActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		spinner_cityObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				userInfo.setCityObj(cityList.get(arg2).getCityNo()); 
			}
			@Override
			...
			public void onClick(View v) {
				try {
					...
					if(ET_address.getText().toString().equals("")) {
						Toast.makeText(UserInfoEditActivity.this, "详细地址输入不能为空!", Toast.LENGTH_LONG).show();
	...
	private void initViewData() {
		...
		for (int i = 0; i < cityList.size(); i++) {
			if (userInfo.getCityObj().equals(cityList.get(i).getCityNo())) {
